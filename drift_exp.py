    ...
    for model_name in args.models:
        ...
        for drift_type in drift_types:
            ...
            if final_layer:
                 ...
                 trainable_params = list(filter(lambda p: p.requires_grad, model_noisy_ft.parameters()))
                 if trainable_params:
                     optimizer_ft_noise = optim.SGD(trainable_params, lr=args.lr_ft, momentum=0.9, weight_decay=5e-4)
                 else:
                     print("Warning: No trainable parameters found in the final layer for noise fine-tuning. Skipping.")
            else:
